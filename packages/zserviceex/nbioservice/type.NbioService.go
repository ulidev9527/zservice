...
func NewNbioService_WebSocket(opt NbioServiceOption_WebScoket) *NbioService_WebSocket {
	name := fmt.Sprint("NbioService_WebSocket-", opt.Port)
	ns := &NbioService_WebSocket{}

	ns.ZService = zservice.NewService(zservice.ServiceOptions{
		Name: name,
		OnStart: func(z *zservice.ZService) {
			...
			engine := nbhttp.NewEngine(nbhttp.Config{
				Network:                 "tcp",
				Addrs:                   []string{fmt.Sprint("0.0.0.0:", opt.Port)},
			...
			z.LogInfof("nbioService Listen on :%v", opt.Port)
		},
	})
	return ns
}
