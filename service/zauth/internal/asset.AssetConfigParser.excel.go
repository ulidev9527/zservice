...
// 解析 Excel 文件
func AssetConfigParser_Excel(fullPath string) (map[string]string, *zservice.Error) {
	// 解析
	...
	if e := func() *zservice.Error {
		...
		if e != nil {
			return zservice.NewError(e).SetCode(zservice.Code_OpenFileErr)
		}
		defer func() {
		...
		if excel.SheetCount == 0 {
			return zservice.NewError("excel sheet count is 0").SetCode(zservice.Code_Zauth_config_ExcelNoContent)
		}

		sheets := excel.GetSheetList()
		rows, e := excel.Rows(sheets[0])
		if e != nil {
			return zservice.NewError(e).SetCode(zservice.Code_Zauth_config_ParserFail)
		}
