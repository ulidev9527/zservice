...
func HasUserByID(ctx *zservice.Context, id uint32) (bool, *zservice.Error) {
	return DBService.HasTableValue(ctx, dbservice.HasTableValueOption{Tab: &UserTable{}, RK: fmt.Sprintf(RK_UserInfo, id), SQLConds: []any{"uid = ?", id}})
}

// 是否存在这个账号
func HasUserByLoginName(ctx *zservice.Context, loginName string) (bool, *zservice.Error) {
	return DBService.HasTableValue(ctx, dbservice.HasTableValueOption{Tab: &UserTable{}, RK: fmt.Sprintf(RK_UserLoginName, loginName), SQLConds: []any{"login_name = ?", loginName}})
}
...
func GetUserByLoginName(ctx *zservice.Context, loginName string) (*UserTable, *zservice.Error) {
	...
	// 更新缓存
	if e := Redis.SetEX(rk, zservice.Uint32ToString(tab.UID), zservice.Time_10Day).Err(); e != nil {
		ctx.LogError(e)
	}
	if e := Redis.SetEX(fmt.Sprintf(RK_UserInfo, tab.UID), zservice.JsonMustMarshalString(tab), zservice.Time_10Day).Err(); e != nil {
		ctx.LogError(e)
	}
...
func (z *UserTable) AddLoginNameAndPassword(ctx *zservice.Context, name, password string) *zservice.Error {
	...
	if has, e := HasUserByLoginName(ctx, name); e != nil {
		...
		return zservice.NewError("user already exist:", name).SetCode(zservice.Code_Zauth_UserAlreadyExist_LoginName)
	}
...
func (z *UserTable) Save(ctx *zservice.Context) *zservice.Error {
	...
	if e := DBService.SaveTableValue(ctx, z, fmt.Sprintf(RK_UserInfo, z.UID)); e != nil {
		return e.AddCaller()
	}
	return nil
}
