...
func Logic_CheckAuth(ctx *zservice.Context, in *zauth_pb.CheckAuth_REQ) *zauth_pb.CheckAuth_RES {
	...
	// 获取和检查 token
	resultRES := &zauth_pb.CheckAuth_RES{Code: zservice.Code_Zauth_Fail}
	// 获取 token
	authToken := &AuthToken{}
	if at, e := GetTokenInfo(ctx, in.Token); e != nil {
		if e.GetCode() != zservice.Code_NotFound { // 其它错误
			ctx.LogError(e)
	...
	if pInfo, e := func() (*PermissionTable, *zservice.Error) {
		...
		if len(tabs) == 0 {
			return nil, zservice.NewError("not found").SetCode(zservice.Code_NotFound)
		}
	...
	case zservice.E_PermissionState_IgnoreAll: // 拒绝所有访问
		ctx.LogWarn("permission is:", zservice.E_PermissionState_IgnoreAll, in)
		resultRES.Code = zservice.Code_Zauth_Fail
	case 1: // 公开访问
	...
	case 2: // 需要登录
		...
		if tab, e := GetPermissionBind(ctx, 2, authToken.UID, permissionInfo.PermissionID); e != nil {
			if e.GetCode() != zservice.Code_NotFound { // 未找到进行父级查找，其它错误直接在这里返回
				ctx.LogError(e)
				resultRES.Code = e.GetCode()
				return resultRES
			}
			...
			if tab.State == 1 {
				...
				resultRES.Code = zservice.Code_Limit
				return resultRES
			}
		}
