...
func HasPermissionByID(ctx *zservice.Context, id uint32) (bool, *zservice.Error) {
	return DBService.HasTableValue(ctx, &PermissionTable{}, fmt.Sprintf(RK_PermissionInfo, id), fmt.Sprintf("permission_id = %v", id))
}

// 根据ID获取一个权限
func GetPermissionByID(ctx *zservice.Context, id uint) (*PermissionTable, *zservice.Error) {
	tab := &PermissionTable{}
	if e := DBService.GetTableValue(ctx, tab, fmt.Sprintf(RK_PermissionInfo, id), fmt.Sprintf("permission_id = %v", id)); e != nil {
		return nil, e
	}
	return tab, nil
}
