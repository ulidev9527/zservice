...
func Logic_SMSVerifyCodeSend(ctx *zservice.Context, in *zauth_pb.SMSVerifyCodeSend_REQ) *zauth_pb.SMSSendVerifyCode_RES {
	...
	if in.Phone == "" || in.Phone[0] != '+' {
		return &zauth_pb.SMSSendVerifyCode_RES{Code: zservice.Code_ParamsErr}
	}
	...
	if isBan, e := IsSmsBan(ctx, in.Phone); e != nil {
		...
		return &zauth_pb.SMSSendVerifyCode_RES{Code: zservice.Code_Zauth_Sms_Phone_Ban}
	}
	...
	if has, e := Redis.Exists(rKeyCD).Result(); e != nil {
		ctx.LogError(e)
		return &zauth_pb.SMSSendVerifyCode_RES{Code: zservice.Code_Fail}
	} else if has > 0 {
		ctx.LogError(zservice.NewError("phone cd", in.Phone))
		return &zauth_pb.SMSSendVerifyCode_RES{Code: zservice.Code_Zauth_Sms_Phone_CD}
	}
	...
	return &zauth_pb.SMSSendVerifyCode_RES{Code: zservice.Code_SUCC, VerifyCode: fmt.Sprint("****", verifyCode[4:])}
}
