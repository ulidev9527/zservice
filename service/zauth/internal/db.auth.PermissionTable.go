...
func SyncPermissionTableCache(ctx *zservice.Context) *zservice.Error {
	return DBService.SyncTableCache(ctx, &[]PermissionTable{}, func(v any) string {
...
func HasPermissionByID(ctx *zservice.Context, id uint32) (bool, *zservice.Error) {
	return DBService.HasTableValue(ctx, dbservice.HasTableValueOption{Tab: &PermissionTable{}, RK: fmt.Sprintf(RK_PermissionInfo, id), SQLConds: []any{"permission_id = ?", id}})
}
...
func GetPermissionBySAP(ctx *zservice.Context, service, action, path string) (*PermissionTable, *zservice.Error) {
	...
	// 未找到 查表
	tab := &PermissionTable{}
	if e := Gorm.Model(&PermissionTable{}).Where("service = ? AND action = ? AND path = ?", service, action, path).First(tab).Error; e != nil {
		if DBService.IsNotFoundErr(e) {
...
func (z *PermissionTable) Save(ctx *zservice.Context) *zservice.Error {
	...
	if e := DBService.SaveTableValue(ctx, z, fmt.Sprintf(RK_PermissionInfo, z.PermissionID)); e != nil {
		return e.AddCaller()
	}
	return nil
}
